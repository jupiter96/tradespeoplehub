    ...
    <div className="space-y-4 md:space-y-6 px-4 md:px-6">
          ...
          <h4 className="font-['Poppins',sans-serif] text-[16px] text-[#2c353f] mb-2">
            {(() => {
              const isAutoCompleted =
                ...
                (currentOrder as any).metadata?.autoApprovedReason === "no_client_response";
              if (isAutoCompleted) {
          ...
          <p className="font-['Poppins',sans-serif] text-[13px] text-[#6b6b6b] mb-4 whitespace-pre-line">
            {(() => {
              const isAutoApproved =
                (currentOrder as any).metadata?.autoApprovedAt ??
                (currentOrder as any).metadata?.autoApprovedDeadlineAt ??
                ((currentOrder as any).metadata?.autoApprovedReason === "no_client_response" ? true : null);
              const deadlineForDisplay =
                (currentOrder as any).metadata?.autoApprovedDeadlineAt ||
                (currentOrder as any).metadata?.autoApprovedAt ||
                ((currentOrder as any).completedDate ? new Date((currentOrder as any).completedDate).toISOString() : null);
      ...
      {currentOrder.status === "disputed" && (() => {
        ...
        if (bothPaid) {
          ...
          statusMessage = `You have paid your arbitration fees to ask our arbitration team to step in and decide the case. ${currentOrder.client || "The client"} has been notified and expected to complete payment before ${arbitrationDeadline}.`;
              ...
              <div key={`${event.id}-${event.at || "na"}-${index}`} className="flex gap-4 mb-0 relative group">
                ...
                <div className="flex-1 pb-6">
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 hover:shadow-md transition-shadow">
                    ...
                    {/* Message/Reason - Highlighted with dynamic styling for warnings */}
                    {event.message && (() => {
