...
type AdminOrdersPageProps = {
  /** Filter by status: Completed, In Progress, cancel-order (Cancelled + Cancellation Pending), disputed, or undefined for all */
  initialStatus?: string;
  title?: string;
  description?: string;
  /** When true, do not show status filter dropdown (page is already status-specific) */
...
const STATUS_OPTIONS: { value: string; label: string }[] = [
  ...
  { value: "cancel-order", label: "Cancelled" },
...
function getStatusBadgeClass(status: string): string {
  const map: Record<string, string> = {
    ...
    "Cancellation Pending": "bg-amber-100 text-amber-700 border-amber-200",
    ...
    "offer created": "bg-gray-100 text-gray-700 border-gray-200",
  };
...
export default function AdminOrdersPage({
  ...
  description = "View and manage orders by status. Search, filter, and sort order details.",
  ...
  useEffect(() => {
    fetchOrders();
  }, [page, sortField, sortDirection, statusFilter, searchQuery]);
        ...
        <div className="rounded-3xl border-0 bg-white p-6 shadow-xl shadow-[#FE8A0F]/20">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="text-black">Loading orders...</span>
            </div>
          ) : orders.length === 0 ? (
                  ...
                  <TableBody>
                    {orders.map((order) => (
                      ...
                      >
                        ...
                        <TableCell className="text-black">
                          <Badge className={`${getStatusBadgeClass(order.status)} border text-xs`}>
                            {order.status}
                          </Badge>
                        </TableCell>
