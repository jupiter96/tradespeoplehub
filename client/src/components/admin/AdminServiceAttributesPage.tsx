...
interface ServiceSubCategory {
  _id: string;
  name: string;
  serviceAttributeSuggestions?: string[];
...
export default function AdminServiceAttributesPage() {
  ...
  // Stores attribute suggestions for each subcategory
  ...
  // Fetch subcategories for a specific level and load their attributes
  const fetchSubCategoriesForLevel = useCallback(async (serviceCategoryId: string, level: number, parentSubCategoryId?: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Load attributes for all subcategories at this level
        const attributesMap: Record<string, string[]> = {};
        for (const subCat of subCategories) {
          attributesMap[subCat._id] = subCat.serviceAttributeSuggestions || [];
        }
  ...
  // Save all attribute suggestions
  const handleSaveAll = async () => {
    ...
    const hasAnyAttributes = Object.values(subCategoryAttributes).some(attributes => attributes.length > 0);
    if (!hasAnyAttributes) {
      toast.error("Please add at least one attribute suggestion");
      return;
    }
    ...
    try {
      ...
      // Update all subcategories that have attributes
      for (const [subCategoryId, attributes] of Object.entries(subCategoryAttributes)) {
          ...
          try {
            const response = await fetch(
              resolveApiUrl(`/api/service-subcategories/${subCategoryId}`),
              {
                ...
                credentials: "include",
                body: JSON.stringify({
                  serviceAttributeSuggestions: attributes
                    .map(attr => attr.trim())
                    .filter(a => a.length > 0)
                }),
              }
      ...
      if (successCount > 0) {
        toast.success(`Successfully saved attribute suggestions for ${successCount} subcategor${successCount === 1 ? 'y' : 'ies'}`);
      }
      if (errorCount > 0) {
        toast.error(`Failed to save ${errorCount} subcategor${errorCount === 1 ? 'y' : 'ies'}`);
      }
    } catch (error) {
      console.error("Error saving service attribute suggestions:", error);
      toast.error("Failed to save service attribute suggestions");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (!categoryId) {
    return (
      <AdminPageLayout
        title="Service Attribute Management"
  ...
  return (
    <AdminPageLayout
      title={`Service Attribute Management - ${serviceCategory.name}`}
      description="Manage service attribute suggestions for each subcategory (displayed in 'What's Included' section)"
    >
      <div className="space-y-6">
        ...
        {/* Attribute Management for Selected Subcategory */}
        {(() => {
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <Label className="text-sm font-semibold text-[#FE8A0F] block">
                    Attribute Suggestions (What's Included)
                  </Label>
              ...
              <div className="space-y-3">
                {attributes.length > 0 ? (
                  attributes.map((attribute, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <Input
                        ...
                        placeholder="e.g., Professional service guaranteed"
                        className="flex-1"
                      />
                      <Button
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-8">
                    No attribute suggestions yet. Click "Add Attribute" to create one.
                  </p>
                )}
              </div>
            </div>
